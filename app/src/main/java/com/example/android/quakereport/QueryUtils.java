    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            for(int i =0;i<features.length();i++){
                JSONObject myfeature = features.optJSONObject(i);
                JSONObject myprop = myfeature.optJSONObject("properties");
                double mag = myprop.getDouble("mag");
                String place = myprop.getString("place");
                long time = myprop.optLong("time");
                Earthquake thisearthquake = new Earthquake(place, mag, time);
